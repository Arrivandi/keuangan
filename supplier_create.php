...
if (isset($_POST['submit'])) {
    ...
    $query = "INSERT INTO supplier_progress 
    (project_code, project_owner, project_name, supplier_name, item, local_import, po_mju_no, po_mju_date, payment_mju_status, cash_out_total, import_number)
    VALUES 
    ('$project_code', '$project_owner', '$project_name', '$supplier_name', '$item', '$local_import', '$po_mju_no', '$po_mju_date', '$payment_mju_status', '$cash_out_total', '$import_number')";
...
<div class="container">
<h2>Tambah Supplier Progress</h2>
<form method="POST">
    ...
    <input type="text" name="po_mju_no">

    <label>PO MJU Date:</label>
    <input type="date" name="po_mju_date">

    <label>Payment MJU Status:</label>
    <select name="payment_mju_status">
        <option value="Unpaid">Unpaid</option>
        <option value="Supp PPN 1">Supp PPN 1</option>
        <option value="Supp PPN 2">Supp PPN 2</option>
        <option value="Finish">Finish</option>
    </select>

    <label>Cash Out Total:</label>
    <input type="number" name="cash_out_total">
